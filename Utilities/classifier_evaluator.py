...
if __name__ == "__main__":
    ...
    parser.add_argument('--probabilistic', action=argparse.BooleanOptionalAction, default=True,
                        help='Whether predictions are probabilistic')
    parser.add_argument('--action', type=str, choices=['eval', 'tht', 'roc', 'pr'], default='eval',
                        help='Compute metrics, find optimal decision threshold, or draw a curve')
    parser.add_argument('--tht_mode', type=str, choices=['roc', 'pr'], default='pr',
    ...
    if args.action == 'eval':
        acc, pre, rec, spe, f1, MCC, AUROC, AUPR = evaluate(y, predictions, y_hat)
