...
def TMTF(args: argparse.Namespace) -> np.ndarray:
    if args.dataset == "DTINET":
        ...
        Create tensors
        """
        script_T = np.concatenate(
            (
                np.expand_dims(protein_sequence_sim, axis=-1),
                np.expand_dims(protein_interactions, axis=-1),
                np.expand_dims(protein_disease_sim, axis=-1),
            ),
            axis=-1,
        )

        script_D = np.concatenate(
            (
                np.expand_dims(drug_structure_sim, axis=-1),
                np.expand_dims(drug_ineractions, axis=-1),
                np.expand_dims(drug_disease_sim, axis=-1),
                np.expand_dims(drug_se_sim, axis=-1),
            ),
            axis=-1,
        )
        ...
        fold_no = 1
        log_file = open("Results/TMTF/log.txt", "w") if args.log else None
        for _, pos_test_split in pos_sample_splits:
            log_file.write("Fold {}\n".format(fold_no)) if args.log else None
            _, neg_test_split = next(neg_sample_splits)

            pos_test_indices = (
                pos_sample_indices[0][pos_test_split],
                pos_sample_indices[1][pos_test_split],
            )
            test_indices = (
                np.concatenate(
                    (
                        neg_sample_indices[0][neg_test_split],
                        pos_sample_indices[0][pos_test_split],
                    ),
                    axis=0,
                ),
                np.concatenate(
                    (
                        neg_sample_indices[1][neg_test_split],
                        pos_sample_indices[1][pos_test_split],
                    ),
                    axis=0,
                ),
            ...
            X_k_hat = dtinet_optimizer.optimize(
                X_k,
                script_D,
                script_T,
                args.form_no,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Whether to log the optimization process or not",
    )
    args = parser.parse_args()

    if not os.path.exists("Results"):
        os.mkdir("Results")
        os.mkdir("Results/TMTF")
    elif os.path.exists("Results") and not os.path.exists("Results/TMTF"):
        os.mkdir("Results/TMTF")

    X_hat = TMTF(args)
    np.savetxt("Results/TMTF/X_hat.txt", X_hat)
