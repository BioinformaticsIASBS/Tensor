...
def TMTF(args: argparse.Namespace) -> np.ndarray:
    if args.dataset == 'DTINET':
        ...
        head = data[0]
        tail = data[1:]
        protein_interactions = np.array(
            [list(map(int, row[1:])) for row in tail])
        ...
        head = data[0]
        tail = data[1:]
        protein_sequence_sim = np.array(
            [list(map(float, row[1:])) for row in tail])/100
        ...
        head = data[0]
        tail = data[1:]
        protein_disease_sim = np.array(
            [list(map(float, row[1:])) for row in tail])
        ...
        pos_sample_splits = KFold(
            n_splits=args.k, shuffle=True).split(pos_sample_indices[0])
        neg_sample_splits = KFold(
            n_splits=args.k, shuffle=True).split(neg_sample_indices[0])
        ...
        for _, pos_test_split in pos_sample_splits:
            ...
            pos_test_indices = (
                pos_sample_indices[0][pos_test_split], pos_sample_indices[1][pos_test_split])
        ...
        Load drug-target matrix
        '''
        file = open('Data/Gold standard dataset/' +
                    args.dataset.lower() + '_admat_dgc.txt')
        ...
        '''
        file = open('Data/Gold standard dataset/' +
                    args.dataset.lower() + '_simmat_dc.txt')
        lines = [line for line in file]
        head = lines[0].split()
        tail = [line.split()[1:] for line in lines[1:]]
        ...
        '''
        file = open('Data/Gold standard dataset/' +
                    args.dataset.lower() + '_simmat_dg.txt')
        lines = [line for line in file]
        head = lines[0].split()
        tail = [line.split()[1:] for line in lines[1:]]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--f_size', type=int, default=16,
                        help='Number of latent variables')
    parser.add_argument('--alpha', type=float,
                        default=0.01, help='Learning rate')
    parser.add_argument('--epsilon', type=float,
                        default=1e-3, help='Stopping criterion')
